...
public class ProductService {
	...
	public ProductService() {
		// Sample product list
		this.products = new ArrayList<>(List.of(new ProductModel(1L, "T-Shirt", 19.99, "M", "Red", "Clothing"),
	...
	public List<ProductModel> deleteProductById(Long id) {
		ProductModel productToRemove = products.stream().filter(product -> product.getId().equals(id)).findFirst()
				.orElse(null);
	...
	public List<ProductModel> getProductsByColor(String color) {
		return products.stream().filter(product -> product.getColor().equalsIgnoreCase(color))
				.collect(Collectors.toList());
	}

	// Method to filter products by category
	public List<ProductModel> getProductsByCategory(String category) {
		return products.stream().filter(product -> product.getCategory().equalsIgnoreCase(category))
				.collect(Collectors.toList());
	}
